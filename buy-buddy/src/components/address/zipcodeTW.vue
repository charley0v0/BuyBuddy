...
selectValue.value = props.postalCode ? `${props.postalCode} ${props.city} ${props.district}` : '';

watch(()=>props.district,()=>{
    selectValue.value = props.postalCode ? `${props.postalCode} ${props.city} ${props.district}` : '';
})

//與父組件v-model綁定
const emit = defineEmits(['update:postalCode', 'update:city', 'update:district', 'blur']);
watch(()=> selectValue.value, () => {
    if(selectValue.value){
        emit('update:postalCode', selectValue.value.split(' ')[0]);
        emit('update:city', selectValue.value.split(' ')[1]);
        emit('update:district', selectValue.value.split(' ')[2]);
    }else{
...
function transform(zipData) {
    return Object.keys(zipData).map(city => {
        const districts = Object.keys(zipData[city]).map(districtName => ({
            id: `${zipData[city][districtName]} ${city} ${districtName}`,
            value: `${zipData[city][districtName]} ${city} ${districtName}`,
            label: `${zipData[city][districtName]} ${city} ${districtName}`,
        }));
        return {
            id: city,
            value: city,
...
const zipCodeList = transform(zipData);
